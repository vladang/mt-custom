...
class MTConnect
  ...
  private function CreateConnection()
    ...
    if(!($this->m_connect = socket_create(AF_INET, SOCK_STREAM, SOL_TCP)))
      ...
      socket_set_nonblock($this->m_connect);

      socket_connect($this->m_connect, $this->m_ip_mt5, $this->m_port_mt5);

      // Эмуляция таймаута подключения
      // socket_set_nonblock выше обязательно
      $time = \time() + 20;
      do
      {
        $isWritten = \socket_write( $this->m_connect, '' );
      }
      while( $isWritten === FALSE && time() <= $time );
      ...
      if ($isWritten === false) {
        ...
        //throw new ModelNotSaveException('Connection error with MetaTrader');
  ...
  public function Disconnect()
    {
    if (is_resource($this->m_connect) && get_resource_type($this->m_connect) === 'Socket')
      {
  ...
  public function SetCryptRand($crypt, $password)
    ...
    $out                = md5(mt5_connect . phpmd5(mb_convert_encoding($password, 'utf-16le', 'utf-8'), true) . MTProtocolConsts::WEB_API_WORD);
    //---
    for($i = 0; $i < 16; $i++)
      {
      $out                = md5(mt5_connect . phpMTUtils::GetFromHex(substr($this->m_crypt_rand, $i * 32, 32)) . MTUtils::GetFromHex($out));
      $this->crypt_iv[$i] = $out;
      }
    }
  }
