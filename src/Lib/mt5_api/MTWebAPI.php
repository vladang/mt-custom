...
class MTWebAPI
{
    ...
    private $fileCounter = "mt_counter.log";
    ...
    private function counterAdd()
    {
        $data = array();
        if (file_exists($this->fileCounter)) {
            $data = json_decode(file_get_contents($this->fileCounter), true);
        }
        $key = date("d_m_Y");
        if (!empty($data[$key])) {
        ...
        //file_put_contents($this->fileCounter, json_encode($data), LOCK_EX);
    }
